...
"""
    gnu += "plot '%s' using (100.*$2/$12):xtic(1) title column(2) lc rgb '%s',\\" %(args.infile, colors[0])
    for i in range(3, 11):
	gnu += "\n     '%s' using (100.*$%d/$12):xtic(1) title column(%d) lc rgb '%s',\\" %(args.infile, i, i, colors[i-2])
    gnu += "\n     '%s' using (100.*$11/$12):xtic(1) title column(11) lc rgb '%s'\n" %(args.infile, colors[9])
